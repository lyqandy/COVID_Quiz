...
export default class NewClass extends cc.Component {
            ...
            if (event.data && event.data.data) {
                ...
                if (event.data.data.gameState.curRound >= 0) {
                    ...
                    if (event.data.data.gameState.curRound === 5 && event.data.data.gameState.curRoundTime === 15000) {
                        // cc.tween(this.double).to(1, { scale: 1 })
                        //     .then(cc.tween(this.double).to(2.5, { opacity: 0 }))
                        //     .start();
                        this.double.active = true;
                        this.double.getComponent(cc.Animation).play('double_effect');
                    }
                    this.titleLabel.string =  c(t("第${0}/6题"), [(event.data.data.gameState.curRound + 1) + ""]); // "第" + (event.data.data.gameState.curRound + 1) + "/6题"
    ...
    reset(data) {
        ...
        for (let i = 0; i < ques.opt.length; i++) {
            // this.ansList[i].overflow = cc.Label.Overflow.RESIZE_HEIGHT;
            // this.ansList[i].fontSize = 36;
            ...
            // if (ques.opt[i].length > 26) {
            //     // this.ansList[i].overflow = cc.Label.Overflow.RESIZE_HEIGHT;
            //     this.ansList[i].fontSize = 30;
            // }
    ...
    countdown() {
        ...
        // let num = Math.floor(i / 1000);
        clearInterval(this.ti);

        // if (num === this.countnumber) {
        //     // return;
        // } else {
        this.countnumber = 15;
        //     clearInterval(this.ti);
    ...
    setScore(own, oth, ownAnsS, otherAnsS) {
        ...
        // this.stateIconR.node.opacity = 255;
        // this.stateIconL.node.opacity = 255;
        ...
        if (otherAnsS === 1) {
            ...
            // cc.tween(this.stateIconR.node).to(2, { opacity: 0 })
            //     .start();
        }
        if (oth !== this.otherS) {
            this.standLabelR.active = false;
        ...
        if (ownAnsS === 1) {
            ...
            // cc.tween(this.stateIconL.node).to(2, { opacity: 0 })
            //     .start();
        }
        if (own !== this.ownS) {
            this.standLabelL.active = false;
