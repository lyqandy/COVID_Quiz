...
export default class ExplanationUI extends cc.Component {
    ...
    renderHeader() {
        ...
        this.correctnessLabel.string = `${t("正确率")}: ${100 - w}%`;
        this.incorrectnessLabel.string = `${t("错误率")}: ${w}%`;
    ...
    renderPoints() {
        ...
        let { questions, mistakes } = Global.assesment;
    ...
    renderExplanation(
        ...
        number.getComponent(cc.Label).string = c(t("第${0}题"), [`${idx + 1}/${total}`]);
    ...
    onPointClick(ev: cc.Event.EventTouch, data: string) {
        ...
        // this._scrollPos.x = 0;
        // this._scrollPos.y = abs(p.y / content.height);
        // this.scrollView.scrollToPercentVertical(
        //     1 - (abs(p.y) - 100) / content.height, 0.75
        // );
