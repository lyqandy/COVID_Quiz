...
export default class PersonalView extends cc.Component {
    ...
    start() {
        ...
        if (typeof wx === 'undefined') {
            ...
            wx.cloud.callFunction({
                name: Global.cloudFunc.getTestQuestions[Math.round(Math.random() * (Global.cloudFunc.getTestQuestions.length - 1))],
                data: { size: 10 },
                success: res => {
    ...
    renderQuestion() {
        ...
        this.progressLabel.string = c(t("第${0}题"), [`${i} / ${l}`]);
    ...
    onFinish() {
        ...
        // const data = JSON.stringify(Global.assesment);
        // cc.sys.localStorage.setItem('AssesmentData', data);
